...
impl HoliumInterplanetaryNodeData {
    pub fn new(data_vec: Vec<u8>) -> Result<Self> {
        let data = sk_cbor::read(&data_vec)
    ...
    fn recursively_read_from_ip_area(cid: &Cid, ip_context: &InterplanetaryContext) -> Result<Value> {
        ...
        if let Ok(scalar_data_envelope) = ScalarDataEnvelope::try_from(*block.clone()) {
            ...
            let scalar_data = ScalarData::read_from_ip_area(&scalar_data_envelope.scalar_data_cid, ip_context)?;
            // unwrap scalar data
            ...
            let recursive_data_block = Value::read_from_ip_area(&recursive_data_envelope.recursive_data_cid, ip_context)?;
            let recursive_data = RecursiveData::try_from(*recursive_data_block)?;
            // recursively read elements from ip area
            let elements = recursive_data.elements_cids
                .iter()
        ...
        match &self.0 {
            Value::Array(vec) => {
                ...
                let elements_cids = vec
                    .iter()
                    .map(|item| HoliumInterplanetaryNodeData(item.clone()).recursively_write_to_ip_area(ip_context))
                ...
                let rec_data_envelope = RecursiveDataEnvelope{ recursive_data_cid: rec_data_cid };
                let rec_data_envelope_cid = Value::from(rec_data_envelope).write_to_ip_area(&ip_context)?;
            ...
            _ => {
                ...
                let scalar_data_envelope_cid = Value::from(scalar_data_envelope).write_to_ip_area(&ip_context)?;
