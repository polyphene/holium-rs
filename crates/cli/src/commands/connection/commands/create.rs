...
use anyhow::{anyhow, Context, Result, Error as AnyhowError};
...
use crate::utils::local::context::helpers::{validate_node_name, build_connection_id};
...
pub(crate) fn handle_cmd(matches: &ArgMatches) -> Result<()> {
    ...
    // get argument values
    let tail_type_str = matches.value_of("tail-type")
    ...
    let head_type_str = matches.value_of("head-type")
    ...
    let tail_type = tail_type_str.parse::<NodeType>().map_err(AnyhowError::msg)?;
    let head_type = head_type_str.parse::<NodeType>().map_err(AnyhowError::msg)?;
